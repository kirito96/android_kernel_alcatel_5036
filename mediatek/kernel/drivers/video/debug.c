...
extern unsigned int mtkfb_fm_auto_test();
extern UINT32 color;
...
//	hdmi_force_init();
    ...
    else if (0 == strncmp(opt, "pll_clk:", 8))
    {
        char *p = (char *)opt + 8;
        unsigned int pll_clk = (unsigned int) simple_strtoul(p, &p, 10);

        if (pll_clk) {
            lcm_params->dsi.PLL_CLOCK = pll_clk;
        } else {
            goto Error;
        }
        DISP_LOG_PRINT(ANDROID_LOG_INFO, "DBG", "pll_clk=%d\n", lcm_params->dsi.PLL_CLOCK);
    }
    else if (0 == strncmp(opt, "pll_ssc:", 8))
    {
        if (0 == strncmp(opt + 8, "on", 2)) {
            lcm_params->dsi.ssc_disable = 0;
        } else if (0 == strncmp(opt + 8, "off", 3)) {
            lcm_params->dsi.ssc_disable = 1;
        } else if (0 == strncmp(opt + 8, "range=", 6)) {
            char *p = (char *)opt + 14;
            unsigned int pll_ssc_range = (unsigned int) simple_strtoul(p, &p, 10);
    
            if (pll_ssc_range) {
                lcm_params->dsi.ssc_range = pll_ssc_range;
            } else {
                goto Error;
            }
        } else {
        ...
        DISP_LOG_PRINT(ANDROID_LOG_INFO, "DBG", "pll_ssc_en=%d, pll_ssc_range=%d\n", lcm_params->dsi.ssc_disable ^ 1, lcm_params->dsi.ssc_range);
    }
