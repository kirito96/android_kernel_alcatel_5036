        ...
        {    
			...
			/* Set CV */
				...
				//	cv_voltage = BATTERY_VOLT_04_350000_V;
					 /*mingquan.lai added for high temp charge ====2013.07.16*/
	     			if ((BMT_status.temperature > 0)&&(BMT_status.temperature <= 38))
            		{
                		cv_voltage = BATTERY_VOLT_04_350000_V;
            		}
            		else if ((BMT_status.temperature > 38)&&(BMT_status.temperature <= 45))
            		{
                		cv_voltage = BATTERY_VOLT_04_350000_V;
            		}
            		else
...
PMU_STATUS BAT_BatteryFullAction(void)
{
...
#endif        	
    {
    	if((BMT_status.temperature >= 45) && (BMT_status.temperature < 55))
    	{
    		if(BMT_status.bat_vol < RECHARGING_HIGH_TEMP_VOLTAGE)
    		{
	...
	if(g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || g_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)
	{
		...
		mt_set_gpio_out(GPIO15, GPIO_OUT_ONE);
	}
#endif
