...
// mingquan.lai adds to fix power-off result from battery temperature checking for YarisL
#if !defined(JRD_CALIFORNIA_BORAD) && defined(JRD_BUILD_MMITEST)
	if(bat_temperature_val >= 60)
		bat_temperature_val = 57;
#endif
...
void dod_init(void)
{
    ...
    {
		//mingquan.lai modify to fix capacity issue
        if(((g_rtc_fg_soc != 0) && ((g_rtc_fg_soc-gFG_capacity_by_v) < 20) &&(( gFG_capacity_by_v > 5 || bat_is_charger_exist() == KAL_TRUE)))
    ...
    {
		//mingquan.lai modify to fix capacity issue
    	if(((g_rtc_fg_soc != 0) && ((gFG_capacity_by_v-g_rtc_fg_soc) < 20) &&(( gFG_capacity_by_v > 5 || bat_is_charger_exist() == KAL_TRUE)))
			|| ((g_rtc_fg_soc != 0) &&(g_boot_reason == BR_WDT_BY_PASS_PWK || g_boot_reason == BR_WDT)))
		{
            gFG_capacity_by_v = g_rtc_fg_soc;            
        }
...
void oam_init(void)
{
	...
	//modify by mtk to fix capacity issue 
    //if(bat_is_charger_exist() == KAL_TRUE)
    {
    	...
    	//modify by mtk to fix capacity issue
    	if(((gFG_capacity_by_v == 100) && (vbat_capacity < 90)) ||(abs(gFG_capacity_by_v-vbat_capacity)>25))	
    	{
