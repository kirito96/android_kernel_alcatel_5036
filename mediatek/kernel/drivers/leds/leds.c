...
extern void led_hw_control_switch(int enable);
...
static int led_breath_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting* led)
{
	...
	switch(pmic_type){
		...
		case MT65XX_LED_PMIC_NLED_ISINK3:
            ...
            upmu_set_isink_dim3_fsel(0X1000); // 1K = 32000 / (0 + 1) / 32
            upmu_set_isink_ch3_step(0x0); // 4mA
            upmu_set_isink_sfstr3_tc(0x0); // 0.5us
            upmu_set_isink_sfstr3_en(0x0); // Disable soft start
			upmu_set_isink_breath3_trf_sel(0x0); // 0.926s
			upmu_set_isink_breath3_ton_sel(0x0); // 0.523s
			upmu_set_isink_breath3_toff_sel(0x06); // 1.417s
...
static int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK1)
	{
		...
		{       
            ...
            #if defined (JRD_CALIFORNIA_BORAD)
            upmu_set_isink_ch1_step(0x5); // add to 24mA by zhengdao

			
            #else
			upmu_set_isink_ch1_step(0x2); // add to 16mA by zhengdao
...
//zrl add for resolve blinking white screen when resume of YarisL begin,130820
#if !defined(JRD_CALIFORNIA_BORAD)
	static int pre_level = 255;
#endif
//zrl add for resolve blinking white screen when resume of YarisL end,130820
...
//zrl add to change the BL brightness to 85% begin,130816
		#if defined(JRD_CALIFORNIA_BORAD)
			level = (level+1) * 85 / 100;
			tmp_level = level;
		#endif
//zrl add to change the BL brightness to 85% end,130816

		//zrl add for resolve blinking white screen when resume of YarisL begin,130820
			//printk("zrl debug in mt65xx_led_set_cust111,level = %d,pre_level = %d.\n",level,pre_level);
		#if !defined(JRD_CALIFORNIA_BORAD)
			if(pre_level == 0)
			{
				mdelay(50);
				//printk("zrl debug in mt65xx_led_set_cust112,level = %d,pre_level = %d.\n",level,pre_level);
			}
                       ...
                       if(notify_led == 1)
                               {
                                       notify_led_enable = 1;
                                       led_hw_control_switch(0);
                                       led_breath_pmic(MT65XX_LED_PMIC_NLED_ISINK3, &led_tmp_seting);   //added by zhengdao for one led
