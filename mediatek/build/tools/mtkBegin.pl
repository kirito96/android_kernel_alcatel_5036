...
#zhm add for the dir( vendor/mediatek/$prj/artifacts/out/$prj) is in link mode.
$tmpfromDir = "out/target/product/$prj/";
#$tmptoDir = "out/target/product/tempprj/";

exit 0, if (!-e $flag_subrel && !-e $flag_custrel);
exit 0, if (-e $flag_subrel && -e $flag_custrel);
#exit 0, if (-d $tmpfromDir);
...
#if (!-e $flag_subrel)
{
	...
	#zhm add for the dir( vendor/mediatek/$prj/artifacts/out/$prj) is in link mode.
 	#system("rm -rf $tmptoDir");
	#if (-d $tmpfromDir)
	#{
	#  system("rsync -av --exclude=.svn --exclude=.git --exclude=.cvs $tmpfromDir $tmptoDir > auto_sync_android.log 2>&1");
	#  system("rm -rf $tmpfromDir");
	#} 
	...
	#if (-d $tmptoDir)
	#{
	#  system("rsync -av --exclude=.svn --exclude=.git --exclude=.cvs $tmptoDir $tmpfromDir > auto_sync_android.log 2>&1");
	#  system("rm -rf $tmptoDir");
	#} 
