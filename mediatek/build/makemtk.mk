...
# Set PHONY
# *************************************************************************
.PHONY : new newall jrdremake remake remakeall clean cleanall \
         ...
         update-api modem-info bindergen clean-modem perso check-sign jrdremakeall
...
MAKESIGNMODEM    :=  make  $(MAKEJOBS) sign-modem $(CMD_ARGU) $(MAKE_DEBUG)

# add by zhoujinggao 2013-04-03
...
jrdremake remake update-api drvgen emigen nandgen: custgen
...
cleanall remakeall jrdremakeall:
...
ANDROID_NATIVE_TARGETS := \
         update-api protectimage \
         cts sdk win_sdk otapackage banyan_addon dist updatepackage \
         snod bootimage systemimage recoveryimage secroimage target-files-package mobileinfoimage \
...
ifeq ($(CUR_MODULE),lk)
	...
	$(hide) echo build other
	$(hide) /usr/bin/perl mediatek/build/tools/jrd_mtkBegin.pl $(PROJECT)	
endif	
endif	
	$(hide) $(REMAKECMD) ACTION= $(CUR_MODULE)
...
# zhm add for improve perso build : not build clang &llvm ,copy prebuild host(bin&lib)

perso:  javaoptgen ptgen
	$(hide) echo $(SHOWTIME) $@ing...
	$(hide) /usr/bin/perl mediatek/build/tools/jrd_mtkBegin.pl $(PROJECT)
	$(hide) mkdir -p ./out/host
	$(hide) date
	$(hide) cp  -a  ./development/jrdtools/persoprbuild/host/ ./out/
	$(hide) date
...
ifeq ($(MTK_DEPENDENCY_AUTO_CHECK), true)
  ...
  ifeq ($(strip $(MTK_EMMC_SUPPORT)),yes)
	$(hide) rm -f $(LOGDIR)/$(PROJECT)/EBR*
	$(hide) rm -f $(LOGDIR)/$(PROJECT)/MBR
	$(hide) cp -f mediatek/misc/EBR* $(LOGDIR)/$(PROJECT)
	$(hide) cp -f mediatek/misc/MBR $(LOGDIR)/$(PROJECT)
     ifeq ($(strip $(MTK_YAML_SCATTER_FILE_SUPPORT)),yes)
	$(hide) rm -f $(LOGDIR)/$(PROJECT)/$(MTK_PLATFORM)_Android_scatter.txt
	$(hide) cp -f mediatek/misc/$(MTK_PLATFORM)_Android_scatter.txt $(LOGDIR)/$(PROJECT)
     else
	$(hide) rm -f $(LOGDIR)/$(PROJECT)/$(MTK_PLATFORM)_Android_scatter_emmc.txt
...
ifneq ($(ACTION), )
	...
	if [ -f $(MKTOPDIR)/$(VERSIONDEF) ]; then \
	  echo -n "#define PRELOADER_VER JRD_VERSION_MARK_" > $(MKTOPDIR)/mediatek/platform/mt6572/preloader/src/init/inc/version.h ;\
	  awk ' /PRELOADER_VER/ { print substr($$NF, 2, $(VERSIONLEN)) }' $(MKTOPDIR)/$(VERSIONDEF) >> $(MKTOPDIR)/mediatek/platform/mt6572/preloader/src/init/inc/version.h ;\
	fi
...
ifeq ($(ACTION), )
	$(hide) /usr/bin/perl mediatek/build/tools/mtkFinalize.pl $(PROJECT) $(MTK_PLATFORM) $(MTK_EMMC_SUPPORT) $(MTK_YAML_SCATTER_FILE_SUPPORT) 
...
#############################################################
define copytoout
mkdir -p $(2);rm -f $(LOGDIR)/$(PROJECT)/$(notdir $(1));cp -f $(1) $(2)
endef
