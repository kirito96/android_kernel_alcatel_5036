...
int get_board_info(void)
{
	...
	print("*******JCH***************preloard::::h1=%d====\n",h1);
	print("*******JCH***************preloard::::h0=%d====\n",h0);
	print("********JCH**************preloard::::l1=%d====\n",l1);
	print("********JCH**************preloard::::l0=%d====\n",l0);  
    boardnum = ((h1 & 1) << 3) | ((h0 & 1) << 2) | ((l1 & 1)<< 1) | (l0 & 1) ;
    print("JCH::preloard:=get board info boardnum=%d====\n",boardnum);  
    switch(boardnum){
      case PCBA_1: 
       rel = 1; 
       print("=JCH::preloard:=get board info PCBA_1 ======\n");  
       break;
      case PCBA_2: 
       rel = 2; 
       print("=JCH::preloard:=get board info PCBA_2 ======\n");  
       break;
      case PCBA_3: 
       rel = 3; 
       print("=JCH::preloard:=get board info PCBA_3 ======\n"); 
       break;
      case PCBA_4: 
       rel = 4; 
       print("=JCH::preloard:=get board info PCBA_4 ======\n"); 
       break;
      default: 
       rel = 0;
       print("=JCH::preloard:=get board info erro!!!!======\n");    
    }
...
static void bldr_pre_process(void)
{
    ...
    platform_pre_init();
//2013-06-11 ying.pang for PR468275 begin
#ifdef PRELOADER_VER
#define xstr(s) str(s)
#define str(s) #s
...
#endif
//2013-06-11 ying.pang for PR468275 end
    #ifdef PL_PROFILING
...
//modify by jch for META mode config
        // g_meta_com_type = META_USB_COM;
        if(!usb_cable_in())
              g_meta_com_type=META_UART_COM;
        else
...
#define PRINTD_PART_POS 0x2a0c0000//0x1500000
#define PRINTD_PART_LEN 0xa00000//0x300000
#define PRINTD_PART_OFFSET (1024*1024*2)
#define PRINTD_PL_SIZE (1024 * 8*2)

#define LOG_BUF_LEN (1 << 18)

__attribute__((section(".bss.uninit"))) char _log_buf[LOG_BUF_LEN]={0};
char *log_buf = _log_buf;
unsigned int log_buf_len = LOG_BUF_LEN;
unsigned int log_buf_pos = 0;
unsigned int log_buf_times = 0;
...
void output_log(void)
{
    blkdev_t    *bdev = NULL;
    part_t      *part = NULL;
    //part_t *part = part_get(PART_PRO_INFO);
    part = part_get(PART_EXPDB);
    //u32 offset = 0;
    u32 size;
    u64 addr;
    ...
    if (NULL == (bdev = blkdev_get(CFG_BOOT_DEV)))
        return;

    size = log_buf_pos + 1;   
    //addr = part->startblk * bdev->blksz + PRINTD_PART_OFFSET  ;
     
    //print("addr(%d) \n",addr);

    addr = PRINTD_PART_POS + PRINTD_PART_OFFSET  ;
    for(i = 0;i < 10; i++)
       print("addr(%d) \n",addr);
    if (blkdev_write(bdev, addr, size, log_buf) != 0) {
        print("[%s] bdev(%d) write error (%s)\n", MOD, bdev->type, part->name);
        print("PART_PRO_INFO: %d, %d\n", part_get(PART_PRO_INFO)->startblk, bdev->blksz);
        print("PART_LK: %d, %d\n", part_get(PART_UBOOT)->startblk, bdev->blksz);
    }
}
#endif
...
void main(void)
{
    ...
    *(volatile unsigned int *)0x10200014 = 0xAAAB;//add by jch for merge the MTK6572 patch P39
...
#ifdef CONFIG_CALIFORNIA_BORAD
	board_id = get_board_info(); //add by jch for board info
    print("JCH : main board_info = %d \n", board_id);//add by jch for board info
#endif
