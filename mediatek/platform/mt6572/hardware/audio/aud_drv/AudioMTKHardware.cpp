...
status_t AudioMTKHardware::SetMatvDigitalEnable(bool enable)
{
    ...
    mAudioDigitalInstance->SetMATVDigitalStatus(enable); //ting.kang add for TV
    if (enable == true) {
        ALOGD("SetMatvDigitalEnable, AUDDRV_ENABLE_ATV_I2S_GPIO");
        ::ioctl(mFd, AUDDRV_ENABLE_ATV_I2S_GPIO);  // Set ATV I2S path

		// Added by Xiaodong.Chen at 2013.7.5 for atv/dtv sound out
        // clock require
        mAudioResourceManager->EnableAudioClock(AudioResourceManagerInterface::CLOCK_AUD_AFE, enable);
        mAudioResourceManager->EnableAudioClock(AudioResourceManagerInterface::CLOCK_AUD_ANA, enable);
        //todo:: here to do MergeInterface loopback
mAudioDigitalInstance->SetMATVI2SConnectStatus(true);
		...
		MatvI2SOut.mI2S_SLAVE = AudioDigtalI2S::SLAVE_MODE; // importent for slave
		MatvI2SOut.mI2S_IN_PAD_SEL = AudioDigtalI2S::I2S_IN_FROM_IO_MUX; // importent for standard i2s
		MatvI2SOut.mI2S_SAMPLERATE = 44100;
		mAudioDigitalInstance->Set2ndI2SIn(&MatvI2SOut);
		mAudioDigitalInstance->Set2ndI2SInEnable(enable);
		mAudioDigitalInstance->SetI2SASRCConfig(enable,44100); // convert to 44100
		mAudioDigitalInstance->SetI2SASRCEnable(enable);
		...
		AdcI2SOut.mI2S_SAMPLERATE = 44100;
		mAudioDigitalInstance->SetI2SDacOut(&AdcI2SOut);
		mAudioDigitalInstance->SetI2SDacEnable(enable);
		mAudioDigitalInstance->SetAfeEnable(enable);
		///
		//mAudioAnalogInstance->AnalogOpen(AudioAnalogType::DEVICE_OUT_SPEAKERR, AudioAnalogType::DEVICE_PLATFORM_MACHINE);
		//mAudioAnalogInstance->SetAnalogGain(AudioAnalogType::VOLUME_SPKL, 9);
		//mAudioAnalogInstance->SetAnalogGain(AudioAnalogType::VOLUME_SPKR, 9);
        mAudioVolumeInstance->SetFmVolume(mAudioVolumeInstance->GetFmVolume());
        if (IsOutPutStreamActive() == false) {
            mAudioResourceManager->StartOutputDevice();
        }
...
mAudioDigitalInstance->SetMATVI2SConnectStatus(false);
		// Added by Xiaodong.Chen at 2013.7.5 for atv/dtv sound out
        if (IsOutPutStreamActive() == false) {
            mAudioResourceManager->StopOutputDevice();
        }
		...
		AudioDigtalI2S MatvI2SOut;
		MatvI2SOut.mI2S_IN_PAD_SEL = AudioDigtalI2S::I2S_IN_FROM_CONNSYS;
		mAudioDigitalInstance->Set2ndI2SIn(&MatvI2SOut);
        mAudioDigitalInstance->SetI2SASRCConfig(enable,44100);// Setting to bypass ASRC
        mAudioDigitalInstance->SetI2SASRCEnable(enable);
