...
status_t AudioMachineDevice::StartSpkTrimFunction()
{
   ...
   // #ifdef USING_EXTAMP_HP
    //#else
     property_get("ro.board_info", value, "PCBA");
     //ALOGD("[legen]----the board is :%s-----",value);
     if(strcmp(value,BOARD_ID_AUDIO_EXT_PA)){
	    ALOGD("[legen]----the board is :%s-----",value);
...
{
	char value[10]; //legen
    //baoqiang add for solve bug-471633, the voice comes out from both headset and speaker when listen music 20130810
#if 0
    #define BUF_LEN 1
    int fd,fd1;
    char rbuf[1],rbuf1[1];
    char wbuf[BUF_LEN]={'0'};
    //char *wbuf1[]={'0'}
    char const*headsetpath="/sys/class/switch/h2w/state";
    char const*fmstatepath="/proc/fm";
    
    fd=open(headsetpath,O_RDWR);
    if(fd<0)
    {
    	 ALOGD("can't open the file: %s !\n",headsetpath);	
    }
    else
    {
    	//ALOGD("success to open the file: %s !\n",headsetpath);
	if(-1==read(fd,rbuf,1))
		ALOGD("[legen]   read the headset file fail\n");
	//else
		//ALOGD("[legen] buf=%s \n",rbuf);
	close(fd);
    }
    fd1=open(fmstatepath,O_RDWR);
    if(fd1<0)
    {
    	ALOGD("can't open the file: %s !\n",fmstatepath);
    }
    ...
    {
    	//ALOGD("success to open the file: %s !\n",fmstatepath);
	if(-1==read(fd1,rbuf1,1))
		ALOGD("[legen]   read the fm file fail\n");
	//else
		//ALOGD("[legen] buf1=%s \n",rbuf1);
	close(fd1);
    }    
#endif
    //baoqiang add for bug-471633 end
    ...
    switch (DeviceType) {
        ...
        case AudioAnalogType::DEVICE_OUT_SPEAKERL:
            ...
            //ALOGD("[legen]----the board is :%s-----",value);
            if(!strcmp(value,BOARD_ID_AUDIO_EXT_PA)){            
		     ALOGD("[legen]--123--the board is :%s--%s---",value,BOARD_ID_AUDIO_EXT_PA);	
	            mLock.unlock ();
...
#if 0
	    if( strncmp(rbuf, wbuf,BUF_LEN) && !strncmp(rbuf1, wbuf,BUF_LEN) ) //legen add for bug-471633
			break;
#endif
           // tell kernel to open device
        ...
        case AudioAnalogType::DEVICE_OUT_SPEAKER_HEADSET_L:
            ...
            //ALOGD("[legen]----the board is :%s-----",value);
            if(!strcmp(value,BOARD_ID_AUDIO_EXT_PA)){
	            ALOGD("[legen]----the board is :%s--%s---",value,BOARD_ID_AUDIO_EXT_PA);
	            mLock.unlock ();
	            AnalogOpen(AudioAnalogType::DEVICE_OUT_HEADSETR);
	            mLock.lock ();
            	}
