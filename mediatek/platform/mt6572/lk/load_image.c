 ...
 *
 **********************************************************/
//add by chenghui.jia@tcl.com for MBR check for PR
static char mbr_default[]={
...
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
0x00,0x00,0x05,0x00,0x00,0x00,0x00,0x04,0x00,0x00,0xff,0xff,0xff,0xff,0x00,0x00,
0x00,0x00,0x83,0x00,0x00,0x00,0x00,0x48,0x00,0x00,0x00,0x50,0x00,0x00,0x00,0x00,
0x00,0x00,0x83,0x00,0x00,0x00,0x00,0x98,0x00,0x00,0x00,0x50,0x00,0x00,0x00,0x00,
0x00,0x00,0x83,0x00,0x00,0x00,0x00,0x6e,0x01,0x00,0x00,0x28,0x13,0x00,0x55,0xaa
};

static int mboot_android_check_mbr_hdr(part_dev_t *dev, char *part_name, char *mbr_hdr)
{
    long len=0;
#ifdef MTK_EMMC_SUPPORT
    u64 addr;
    ...
    printf("JCH:######part page addr is 0x%x\n", addr);
    
    len = dev->read(dev, addr, (uchar*) mbr_hdr, 512);
    if (len < 0) {
        printf("JCH:[%s] %s mbr header read error. LINE: %d\n", MODULE_NAME, part_name, __LINE__);
        return -1;
    }    

       printf("\n============================================================\n");
       
    printf("JCH:[%s] Android Partition Name                : %s\n",MODULE_NAME,part_name);
       
    printf("[MBR] JCH: [%s] Android MBR Hdr - from 0x1be          : \n",MODULE_NAME);
       for(i=0x1be;i<0x1ff;i++){
               printf("0x%x,",mbr_hdr[i]);
               if(i%16==0 && i>0)
                       printf("\n");
       }
       printf("\nJCH:============================================================\n");
       
    if(memcmp(mbr_hdr,mbr_default,512)){
               printf("============================================================\n");
               printf("JCH[%s] MBR is changed,now recovery to default MBR\n",MODULE_NAME);
               len = dev->write(dev, (uchar*) mbr_default,addr,  512);
               if (len < 0) {
                       printf("JCH:[%s] %s mbr  write error. LINE: %d\n", MODULE_NAME, part_name, __LINE__);
                       return -1;
               }
       }else{
               printf("JCH:[%s] MBR is same with default value\n",MODULE_NAME);
       }
    return 0;
}

int mboot_check_MBR(char *part_name)
{
    long len;
       unsigned long start_addr;       
    part_t *part;      
    ...
    if (!dev)
    {    
               printf("JCH:mboot_android_load_mbr, dev = NULL\n");
        return -ENODEV;
    }
    ...
    if (!part)
    {
               printf("JCH:mboot_android_load_mbr (%s), part = NULL\n",part_name);
       return -ENOENT;
    }

    start_addr = part->startblk * BLK_SIZE;    

    mbr_hdr = (char*)malloc(512);
    if (!mbr_hdr)
       {
       printf("JCH:mboot_android_load_bootimg_hdr, boot_hdr = NULL\n");
       return -ENOMEM;
    }
       
    len = mboot_android_check_mbr_hdr(dev, part_name, mbr_hdr);
