...
//modify by zhiping.liu for backlight work in lk @start-1
static int brightness_mapto64(int level)
{
        if (!level)
                return 0;
...
//modify by zhiping.liu for backlight work in lk @end-1
#if 0
...
//	printf("[LEDS]LK: mt65xx_led_set_cust: mode=%d, level=%d \n\r", cust->mode, level);
	...
	switch (cust->mode) {
		...
		case MT65XX_LED_MODE_PWM:
			...
			{
			//modify by zhiping.liu for backlight work in lk @start-2
			    return brightness_set_pwm(cust->data, brightness_mapto64(level), &cust->config_data);
			//modify by zhiping.liu for backlight work in lk @end-2
