...
ready:
    ...
    printk(KERN_DEBUG "[EMMC_OTP]use id offset 0x%x\n", p->Offset);
    g_id_offset.cached = 1;
}

// add by xiaoyong.wu for check imei 

static bool check_unavailable_imei(unsigned char *buff_bcd)
{
	unsigned char IMEI_checksum = 0;
	unsigned int BCD_check = 0;
	unsigned int i;
	unsigned int even_num,odd_num;

	//IMEI of BCD code is 0~7 bytes 
	IMEI_checksum = buff_bcd[7] & 0xF;

	printk("IMEI_checksum is: %d   buff_bcd[7] is %x\r\n",IMEI_checksum,buff_bcd[7]);	
	
	for(i=0;i<7;i++)
	{
		//printk("--------buff_bcd[%d] is %x--------------------------------------\r\n",i,buff_bcd[i]);
		//printk("-------%d-------------------------------------%d-------------------------------------\r\n",buff_bcd[i] & 0xF,buff_bcd[i]>>4);
		//1~7 bytes : check if even_num < 10 , if not so make it minus 9 ;then add last all (high 4 bits)
		odd_num =  buff_bcd[i] & 0xF;
		even_num  =	(buff_bcd[i]>>4)*2;
		BCD_check += ( even_num < 10 ? even_num : (even_num-9))+ odd_num;
	}
	
	// the 7th bytes 's high 4 bits is checksum, so minus it and  add the 0th bytes's high 4 bits 	
	//BCD_check = BCD_check + (buff_bcd[0] >>4) - (buff_bcd[7] >>4);
	
	//avoid 00000000000000 IMEI
	if(0 == BCD_check)
	{
		printk("000000000000000  IMEI\r\n");
		return true;
	}
	
	printk("BCD_check is:  %d/r/n",BCD_check);
	//check the sum %10 ,if result ==0  ,check_sum is 0,or checksum = 10-sum %10.e.g  sum 54: checksum =6
	BCD_check = (10 - BCD_check %10)%10;
	//check if checksum_result == the 7th bytes 's high 4 bits
	if (BCD_check == IMEI_checksum)
		return false;
	else
		return true;
}
...
static long mt_otp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case EMMC_OTP_READ:
		...
		//add by xiaoyong.wu if group2's imei is unavailable ,read from group1
		if (check_flag == 1)
		{
			printk("found unavailable IMEI----has fixed offset in id_offset\r\n");
			goto skip;
		}
		if(otpctl.Offset == 0x80)
        {
		    if(check_unavailable_imei(pbuf))
		    {
			    printk("------------------------unavailable IMEI-----g_id_offset.group is %x-------------------------------------------\r\n",g_id_offset.group);
			    check_flag = 1;
			    otpctl.Offset = otpctl.Offset - g_id_offset.group*0x80;
			    g_id_offset.group = 0x0;
			    mt_otp_access(FS_EMMC_OTP_READ, otpctl.Offset, pbuf, otpctl.Length, &otpctl.status);
		    }
        }
