...
#if defined(MT6573) || defined(MT6575) || defined(MT6577) || defined(MT6572)
...
static int bma_init_client(struct i2c_client *client, int reset_cali)
{
	...
	err = bma_set_datarate(client,
		(enum BMA_DATARATE_ENUM)BMA_DATARATE_25HZ);
	if (err < 0) {
		...
		void *buff_out, int size_out, int *actualout)
{
	...
	if ((buff_in == NULL) || (size_in < sizeof(int))) {
		...
		mdelay(1);//add by hugo for waiting data update in G-sensor IC register
	break;
