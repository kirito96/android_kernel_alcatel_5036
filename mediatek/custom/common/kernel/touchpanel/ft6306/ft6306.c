...
//BEGIN: fangjie move from macro UPGRADE_FIRMWARE to the outside for MMITest rawdata test.
typedef unsigned char         FTS_BYTE;     //8 bit
static FTS_BYTE *dma_buffer = NULL;//FTS_BYTE=u8;
static u32 dma_handle = NULL;
static struct mutex dma_model_mutex;
//END: fangjie move from macro UPGRADE_FIRMWARE to the outside for MMITest rawdata test.
...
static unsigned char CTPM_FW_YARISL_FT6306[]=
{
	#include "YarisL_ID80_V35_20131005_app.i"
...
int fts_ctpm_fw_upgrade_with_i_file(void)
{
	...
	if(tp_vendor == 0x80){
		pbt_buf = CTPM_FW_YARISL_FT6306;
		ui_sz = sizeof(CTPM_FW_YARISL_FT6306);
	}
...
unsigned char fts_ctpm_get_i_file_ver(void)
{
	...
	if (ui_sz > 2)
	{
		...
		if(tp_vendor == 0x80)
		return CTPM_FW_YARISL_FT6306[ui_sz - 2];
	}
	else
		//TBD, error handling?
...
int fts_ctpm_auto_upg(unsigned char tp_vendor)
{
	...
	unsigned char uc_tp_fm_ver;
    	unsigned char version_list_yarisl_ft6306[] = {0x0,0x10,0x11,0x30,0x31,0x32,0x33,0x34,0x35};	//Keep the ft6306 tp firmware old version list that allows to be updated.
	int i,i_ret = -1;
	...
	if(tp_vendor == 0x80)
	{
		...
		TPD_DMESG("[FT6306] tp vendor is yeji(0x%x),uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",tp_vendor, uc_tp_fm_ver, uc_host_fm_ver);
		#if 0
		for(i = 0;i < sizeof(version_list_yarisl_ft6306)/sizeof(version_list_yarisl_ft6306[0]); i++)
		{
		//	if(uc_tp_fm_ver<uc_host_fm_ver)//llf 20130222
		//	{
				if(uc_tp_fm_ver == version_list_yarisl_ft6306[i])  
				{
					TPD_DMESG("[FT6306]  tp firmware have new version\n");
					i_ret = fts_ctpm_fw_upgrade_with_i_file();
					if(!i_ret)
						TPD_DMESG("[FT6306]  tp firmware upgrade to new version 0x%x\n", uc_host_fm_ver);
					else
						TPD_DMESG("[FT6306]  tp firmware upgrade failed ret=%d.\n", i_ret);
					break;
				}
		//	}
		//	else
		...
		#endif
		//BEGIN: add by fangjie 
		//because yarisl  only has one FT6306 module (yeji), 
...
static int fts_debug_write(struct file *filp, const char __user *buff, unsigned long len, void *data)
{
	...
	case PROC_UPGRADE:
		{
			...
			ret = fts_ctpm_fw_upgrade_with_app_file(i2c_clientft, upgrade_file_path);
	...
	case PROC_READ_REGISTER:
		writelen = 1;
		TPD_DMESG("%s:register addr=0x%02x\n", __func__, writebuf[1]);
...
static int fts_debug_read( char *page, char **start,off_t off, int count, int *eof, void *data )
{
	...
	case PROC_UPGRADE:
		...
		regaddr = 0xA6;
		ret = ft5x0x_read_reg( regaddr, &regvalue);
		if (ret < 0)
...
*/
	case PROC_READ_DATA:                                                              
		if(count>255){
			printk("%s,%d:PROC_READ_DATA SIZE OUT OF RANGE 255\n",__func__,__LINE__);  
		}
		readlen = count;                                                             
		ret = byte_read( buf, readlen);                        
		if (ret < 0) {                                                               
...
{
	printk("%s,[FT6306] OPEN MMI TEST \n",__func__);
	fts_proc_entry = create_proc_entry(PROC_NAME, 0666, NULL);
	if (NULL == fts_proc_entry) {
		dev_err(&client->dev, "Couldn't create proc entry!\n");
		return -ENOMEM;
	} else {
		dev_err(&client->dev, "Create proc entry success!\n");
...
static int ft5x0x_GetFirmwareSize(char *firmware_name)
{
	...
	printk("[ft5436_llf] Get firmware size begain\n");
	sprintf(filepath, "%s", firmware_name);
			       ...
			       unsigned char *firmware_buf)
{
	...
	printk("[ft5436_llf] Read firmware begain!\n");
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s", firmware_name);
	if (NULL == pfile)
		pfile = filp_open(filepath, O_RDONLY, 0);
				       ...
				       char *firmware_name)
{
	...
	printk("[ft5436_llf] Get firmware size ok size=%d \n",fwsize);
	if (fwsize <= 0) {
		dev_err(&client->dev, "%s ERROR:Get firmware size failed\n",__func__);
		return -EIO;
	}
	...
	printk("[ft5436_llf] Get firmware file ok\n");

	printk("[ft5436_llf] File just is 0x%x 0x%x 0x%x \n",(pbt_buf[fwsize - 8] ^ pbt_buf[fwsize - 6]),
		(pbt_buf[fwsize - 7] ^ pbt_buf[fwsize - 5]),(pbt_buf[fwsize - 3] ^ pbt_buf[fwsize - 4]));
	...
	if ((pbt_buf[fwsize - 8] ^ pbt_buf[fwsize - 6]) == 0xFF
		...
		&& (pbt_buf[fwsize - 3] ^ pbt_buf[fwsize - 4]) == 0xFF) {
		/*call the upgrade function */
		printk("[ft5436_llf] Begain to Upgrade firmware \n");
		i_ret = fts_ctpm_fw_upgrade(pbt_buf, fwsize);
		if (i_ret != 0){
			kfree(pbt_buf);
			dev_dbg(&client->dev, "%s() - ERROR:[FTS] upgrade failed..\n",__func__);
		...
		dev_dbg(&client->dev, "%s:FW format error\n", __func__);
		printk("[ft5436_llf] Firmware file format error\n");
		kfree(pbt_buf);
		return -EIO;
	}
	return i_ret;
...
static ssize_t ft5x0x_fwupgradeapp_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	...
	printk("[ft5436_llf]Input file name is %s \n",fwname);
	...
	fm_ver = ft5x0x_read_fw_ver();
	//i2c_smbus_read_i2c_block_data(i2c_clientft, 0xA6, 1, &fm_ver);
	printk("[ft5436_llf] current firmware version is  0x%x\n", fm_ver);
	
	ft5x0x_read_reg(0xA8, &tp_vendor);
	//i2c_smbus_read_i2c_block_data(i2c_clientft, 0xA8, 1, &tp_vendor);
	//ID detect 
	if((tp_vendor != 0x5d) && (tp_vendor != 0x53) && (tp_vendor != 0x80)&& (tp_vendor != 0x85)&& (tp_vendor != 0x87)){
		//destory #######################################
		mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
		printk("[%s] can't find ft5436,vendor:0x%x\n",__func__,tp_vendor);
		return -1;
	}
	...
	printk("[ft5436_llf] Firmware Upgrade Successful!\n");

	fm_ver = ft5x0x_read_fw_ver();
	
	printk("[ft5436_llf]  After Upgrade firmware version is  0x%x\n", fm_ver);
...
static ssize_t fts_tpfwver_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	u8 fwver = 0;
	struct i2c_client *client = i2c_clientft;

	mutex_lock(&g_device_mutex);

	fwver=ft5x0x_read_fw_ver() ;

	if (fwver< 0)
		num_read_chars = snprintf(buf, 255, "get tp fw version fail!\n");
	else
		num_read_chars = snprintf(buf, 255, "%02X\n", fwver);
...
*/
/*
static ssize_t fts_ftsgetprojectcode_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	ssize_t num_read_chars = 0;
	int projectcode; 
	struct i2c_client *client = i2c_clientft;
	...
	mutex_lock(&g_device_mutex);
	projectcode=fts_ctpm_get_panel_factory_setting(); 
	if(projectcode< 0)
		num_read_chars = snprintf(buf, 255, "get projcet code fail!\n");
	else
		num_read_chars = snprintf(buf, 255, "projcet code = %s\n", projectcode);

	mutex_unlock(&g_device_mutex);
	return num_read_chars;
}

static ssize_t fts_ftsgetprojectcode_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	// place holder for future use 
    return -EPERM;
}
...
static int fts_debug_read1( char *page, char **start,off_t off, int count, int *eof, void *data )
{
	int ret = 0, err = 0;
	u8 tx = 0, rx = 0;
	int i, j;
...
{
	fts_proc_entry1 = create_proc_entry(PROC_NAME1, 0777, NULL);
...
//BEGIN: modify by fangjie for PR741105 
#if 0
...
//END: modify by fangjie for PR741105 
...
/*
	...
	if((tp_vendor == 0x5a) || (tp_vendor == 0x53) || (tp_vendor == 0x80)|| (tp_vendor == 0x85))
	{
			tpd_load_status = 1;
			//tp_pls_status = 1;
			TPD_DMESG("[FT6306][The CTP is FT6306 ---llf] 0xA8 current version value is  0x%x\n", tp_vendor);
	}
	else
	{
			tpd_load_status =0;
			TPD_DMESG("[FT6306][The CTP is MSG2133 ---llf] 0xA8 current version value is  0x%x\n", tp_vendor);
	}
*/
...
//BEGIN: fangjie add for MMITest.apk Rawdata test. 20140626
#ifdef FTS_MMI_DEBUG
	fts_create_proc_boot_status(i2c_clientft);
	fts_create_sysfs(i2c_clientft);
...
//END: fangjie add for MMITest.apk Rawdata test. 20140626
 ...
 static int __devexit tpd_remove(struct i2c_client *client) 
 {
	//BEGIN: fangjie add for MMITest.apk Rawdata test. 20140626
		...
		fts_release_sysfs(i2c_clientft);
	#endif
	//END: fangjie add for MMITest.apk Rawdata test. 20140626
