...
#define JOHN_ADD_ATTR 1   //added by zhengdao
...
GET_THRE:
			...
			ps_data_cali->close = prox_mean+100; //  60  this can customize, adjust close and farway distance
			ps_data_cali->far_away=prox_mean+30;   //   20
			ps_data_cali->valid = 1;
...
GET_THRE:
			...
			ps_data_cali->close = prox_mean+100; //  60this can customize, adjust close and farway distance
			ps_data_cali->far_away=prox_mean+30;  //  20
			ps_data_cali->valid = 1;
			APS_LOG("tmd2772_read_data_for_cali close  = %d,far_away = %d,valid = %d, ofs = %d",ps_data_cali->close,ps_data_cali->far_away,ps_data_cali->valid, ps_data_cali->offset);
			return err; //Do not go through to EXIT_ERR
...
static int tmd2772_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	buffer[0]= TMD2772_CMM_OFFSET;
	err = i2c_master_send(client, buffer, 0x1);

	err = i2c_master_recv(client, offset_test, 0x1);


	printk("offset_test=%d",  offset_test[0]);
